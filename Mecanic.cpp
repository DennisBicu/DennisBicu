#include "Mecanic.h"
Mecanic::Mecanic(string nume2, string prenume2, Data data_n2, Data data_a2)
{
    ID=id_count++;
    coeficient=1.5;
...
int Mecanic::ani_vechime()
{
    return (data_angajare.getAzi()).getAn()-data_angajare.getAn();
}
void Mecanic::afisare(ostream &out)
{
    out<<"ID: "<<ID<<endl;
...
void Mecanic::citire(istream &in)
{
    cout<<"Nume: ";
    string lastName;
    do
...
void Mecanic::set_angajat(string nume2, string prenume2, Data data_n2, Data data_a2)
{
    if((nume2.length()>0 && nume2.length()<=30) && (prenume2.length()>0 && prenume2.length()<=30))
    {
        if(data_n2.check_18(data_a2))
